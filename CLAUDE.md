...
- The fork is on branch `feature/ca65-language-server`, kept rebased onto `oraios/serena` main (force-push after each rebase). **Don't `git add -A` inside `/Users/someone/Documents/serena/`** — claude-code-rig drifts `.serena/memories/*` to symlinks and `.gitignore`, which would silently land in a fork commit. Stage explicit paths.
