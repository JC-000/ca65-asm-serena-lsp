...
**All five milestones done; upstream PR open and CI-green; M5 awaits maintainer review.**
...
- ✅ M5 — **PR [oraios/serena#1504](https://github.com/oraios/serena/pull/1504)** open. Branch `JC-000/serena@feature/ca65-language-server` at `c1a4b4de`. All CI checks green (tests on all 3 OSes, mypy, format/lint, CodeQL).
...
- The fork is on branch `feature/ca65-language-server`, already rebased onto `oraios/serena` main. **Don't `git add -A` inside `/Users/someone/Documents/serena/`** — claude-code-rig drifts `.serena/memories/*` to symlinks and `.gitignore`, which would silently land in a fork commit. Stage explicit paths.
