...
12. **The rebase conflicts are now down to one.** `src/serena/resources/project.template.yml` conflicts every time (its language grid is generated) — resolve by running `.venv/bin/python scripts/print_language_list.py` in the fork and replacing the block between the `# list of language servers` header and the `(This list may be outdated` footnote. The **CHANGELOG.md conflict is gone for good**: the fork's entry was dropped during the 2026-09-03 rebase, so commit `a4e38061` no longer touches that file. Don't add one back.
13. **`scripts/gate.sh` is the pre-change check** for anything touching ca65-ls, the shim, or the hook: hook suite, unit, corpus contract, through-Serena, fork e2e. Red tests are `xfail(strict=True, raises=AssertionError)`; a fix makes them XPASS and fails the gate until the marker is removed. See `docs/red-green-gate.md`.
